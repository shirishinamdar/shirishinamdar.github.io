...
This walkthrough documents the configuration of **UFW** (Uncomplicated Firewall) as a host-based packet filter on a Kali Linux virtual machine. UFW is a frontend over `iptables` that compresses common firewall idioms — default policies, per-service allow rules, per-port rules — into a much shorter command syntax. The objective was to set a default-deny ingress posture on a single host, explicitly allow only the services the host actually needs to expose, and confirm how each UFW command translates into the underlying `iptables` chain entries.
